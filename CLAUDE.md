...
- **Ranking metrics** — `RANKING_METRICS` object defines four sort modes: `overall` (composite score), `cheaper` (lowest price), `fastest` (shortest lead time), `lowest_risk` (lowest risk score). `getRankedShortlistForMetric()` re-sorts the shortlist. `syncActiveShortlist()` updates `currentSupplierShortlist` when metric changes. `renderRankingSwitcher()` renders metric toggle buttons inside the ranking table. `getActiveRankedOutput()` decorates output with `ui_ranked_shortlist`, `ui_top_supplier`, `ui_official_top_supplier`, `ui_metric_meta`.
- **Top Recommendation pinned to ship supplier** — `renderTopRecommendationVisual()` always shows `getCanonicalShipSupplier()` as the hero card, not the metric-sorted #1. Changing the ranking metric only re-sorts the ranking table and alternatives — the hero card stays pinned. The supplier only changes if the user explicitly requests it via chat (which sets `explicitShipSupplierName`).
