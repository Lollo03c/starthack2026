...
- **Status banner** — `buildStatusBannerHtml()` handles three states: green (`can_proceed`), amber (`proceed_with_conditions`), red (`cannot_proceed`). Also accepts legacy `"proceed"` as green for backwards compat. Bug fixed: engine was emitting `"proceed"` instead of `"can_proceed"` — banner checked strict equality so always showed red even after escalations resolved. Fixed in `engine/output_builder.py` line ~519.
