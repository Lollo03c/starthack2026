...
- **Floating chat input bar** — `#chat-input-bar` is absolutely positioned within `#chat-area` (`position: relative`). In hero state, centered at `bottom: 38%`. On submit, `activateChatMode()` adds `.chat-mode` class which transitions to `bottom: 1rem` (0.5s cubic-bezier). Inner `.input-pill` div provides the ChatGPT-style pill look (white bg, `border-radius: 1.5rem`, shadow). Textarea has no border (pill wrapper provides it), send button is `rounded-full`. `#messages` has `pb-24` to clear the floating bar. `resetToInitial()` removes `.chat-mode` to animate bar back to center.
